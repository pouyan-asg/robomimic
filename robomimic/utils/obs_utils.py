...
def initialize_obs_utils_with_obs_specs(obs_modality_specs):
    """
    What is a modality? A modality is a type or category of observation data.
    Examples:
        "low_dim": Low-dimensional data (e.g., robot joint positions, velocities, etc.)
        "rgb": RGB images (camera data)
        "depth": Depth images
        "scan": Lidar or scan data
    
    What is a key? A key is the name of a specific observation in your data.
    Examples:
        "robot0_eef_pos": End-effector position (low_dim)
        "agentview_image": RGB image from a camera (rgb)
        "robot0_eye_in_hand": Another camera image (rgb)
        "robot0_eef_quat": End-effector orientation (low_dim)
    """
    ...
    a list of observation keys under that modality (i.e. OBS_MODALITIES_TO_KEYS), and (2) that maps the inverse, specific
    observation keys to their corresponding observation modality (i.e. OBS_KEYS_TO_MODALITIES).
    ...
    Why is this needed?
        Your model needs to know how to process each type of observation (e.g., images vs. vectors).
        These mappings allow the code to automatically select the right processing 
        function for each observation key.
    ...
    # Ensures the input is always a list for uniform processing.
    ...
    print("\n\033[93m============= Initialized Observation Utils with Obs Spec =============\033[0m\n")
...
def initialize_obs_utils_with_config(config):
    ...
    The objective of this function is to:
        - Parse your config to figure out which observation modalities and keys are present (e.g., images, low-dimensional vectors, depth, scan).
        - Initialize global mappings so that the rest of the code knows how to process each observation type correctly.
        - Set up default encoder settings for each modality, so neural networks can use the right encoder parameters.
    
    In short:
    initialize_obs_utils_with_config reads your config, figures out what types of observations you have, 
    and sets up everything needed for Robomimic to process those observations correctly for your experiment.
    """
