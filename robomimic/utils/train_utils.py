...
def get_exp_dir(config, auto_remove_exp_dir=False, resume=False):
    ...
    Returns:
        ...
        model_dir (str): path to created models directory (sub-folder in experiment directory)
    ...
    t_now = time.time()
    time_str = datetime.datetime.fromtimestamp(t_now).strftime('%Y%m%d_%H%M')
    ...
    base_output_dir = os.path.join(base_output_dir, config.experiment.name+ f"_{time_str}")
    if resume:
    ...
    if config.experiment.save.enabled:
        ...
        os.makedirs(model_dir, exist_ok=resume)
    ...
    # mother_dir = os.path.join(base_output_dir, time_str)
    
    return base_output_dir, log_dir, model_dir, video_dir
