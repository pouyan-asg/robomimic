...
"""
ME:
(robomimic_venv) pouyan@pouyan-XPS-15-9500:~/phd/imitation_learning/robomimic
python robomimic/scripts/train.py --config /home/pouyan/phd/imitation_learning/robomimic/robomimic/exps/templates/bc.json --name bc_vanila --dataset /home/pouyan/phd/imitation_learning/robomimic/datasets/can/mh/low_dim_v15.hdf5 
...
def train(config, device, resume=False):
    ...
    torch.set_num_threads(2)  # This line sets the number of CPU threads that PyTorch will use for parallelizing CPU operations

    print("\n\033[93m============= New Training Run with Config =============\033[0m")
    print(config)
    print("")
    base_dir, log_dir, ckpt_dir, video_dir = TrainUtils.get_exp_dir(config, resume=resume)

    # path for latest model and backup (to support @resume functionality)
    latest_model_path = os.path.join(base_dir, "last.pth")
    latest_model_backup_path = os.path.join(base_dir, "last_bak.pth")
    ...
    for dataset_cfg in config.train.data:
        ...
        print("\n\033[93m============= Loaded Environment Metadata =============\033[0m")
        env_meta = FileUtils.get_env_metadata_from_dataset(dataset_path=dataset_path)

        # populate language instruction for env in env_meta
        # This ensures that every environment metadata dictionary (env_meta) has 
        # a "lang" field, which can be used for language instructions or descriptions
        # in language-conditioned tasks. If no language is specified, it uses "dummy" as a placeholder.
    ...
    if config.experiment.rollout.enabled:
        ...
        for env_i in range(len(env_meta_list)):
            ...
            for env_name in env_names:
                ...
                envs[env_key] = env
                print("\nCreated environment {} for dataset {}\n".format(env_name, env_key))
                print(env)
    ...
    print("\n\033[93m============= Training Dataset =============\033[0m")
    print(trainset)
    print("")
    if validset is not None:
        print("\n\033[93m============= Validation Dataset =============\033[0m")
        print(validset)
        print("")
    ...
    print("\n\033[93m============= Model Summary =============\033[0m")
    print(model)  # print model summary
    print("")

    # print all warnings before training begins
    print("*" * 50)
    print("\033[91mWarnings generated by robomimic have been duplicated here (from above) for convenience. Please check them carefully.\033[0m")
    flush_warnings()
    print("*" * 50)
    print("")
    ...
    # best_return is a dictionary where each key is an environment name, and the value is 
    # initialized to negative infinity (-np.inf). This will store the highest return (score) achieved so far for each environment.
    best_return = {k: -np.inf for k in envs} if config.experiment.rollout.enabled else None
    # best_success_rate is a dictionary where each key is an environment name, and the value is 
    # initialized to -1. This will store the highest success rate achieved so far for each environment.
...
def main(args):
    ...
    if args.config is not None:
        ...
        config = config_factory(ext_cfg["algo_name"])  # only a base instance which needs to be updated
        # Allows you to safely update existing config values from an external config (external json);
        # this will throw errors if the external config has keys not present in the base algo config
        with config.values_unlocked():
    ...
    if args.dataset is not None:
        config.train.data = [{"path": args.dataset}]  # e.g. [{'path': 'datasets/can/ph/low_dim_v15.hdf5'}]
